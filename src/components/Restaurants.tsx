    ...
    <>
      <div className="grid grid-cols-4 w-11/12">
        {props?.restaurantsList
          ?.filter(
            (data: any) =>
              data?.cuisine[0]?.name.includes(props?.menu) ||
              data?.cuisine[1]?.name.includes(props?.menu) ||
              data?.cuisine[2]?.name.includes(props?.menu)
          )
          .map((data: any) => {
            return (
              <>
                <Link to="/details" state={{ data: data }}>
                  <div className="max-w-sm overflow-hidden mt-4">
                    <img
                      className="w-52 rounded-xl h-36 shadow-sm"
